 ...
 */
async function getQuestionBanksByCategory(categoryId) {
  try {
    const snapshot = await db.ref(`questions/${categoryId}`).once('value');
 ...
 */
async function getBankInfo(categoryId) {
  try {
    ...
    if (isDailyQuiz) {
      ...
      const categorySnapshot = await db.ref(`questions/${categoryId}`).once('value');
 ...
 */
async function addQuestionsToCategory(categoryId, questions, bankId = null) {
  try {
    const isDailyQuiz = categoryId === 'daily-quiz';
    
    // Handle Daily Quiz separately (only bank1, max 10 questions)
    if (isDailyQuiz) {
      const bankRef = db.ref(`questions/${categoryId}/bank1`);
    ...
    const bankRef = db.ref(`questions/${categoryId}/${bankId}`);
      const bankSnapshot = await bankRef.once('value');
